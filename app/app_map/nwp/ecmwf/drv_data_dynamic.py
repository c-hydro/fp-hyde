        ...
        if not os.path.exists(file_path_anc_raw_step):
            ...
            for file_data_tag, file_data_fields in file_collections_src.items():
                ...
                if (folder_name_src_tmpl is not None) and (file_name_src_tmpl is not None):
                    ...
                    # info end time step
                    alg_logger.info(' -----> Data "' + file_data_tag + '" ... SKIPPED. File folder or name not defined')
    ...
    def analyze_data(self):
        ...
        if not os.path.exists(file_path_anc_def_step):
            ...
            if os.path.exists(file_path_anc_raw_step):
                ...
                alg_logger.info(' -----> (3) Transform datasets ... ')
                # compose datasets
                obj_data_anc_ds2data_step, obj_attrs_anc_step = transform_dset_2_data(
                    obj_data_anc_extract_step, obj_attrs_anc_extract_step)
                # info start transform datasets
                alg_logger.info(' -----> (3) Transform datasets ... DONE')

                # info start resample datasets
                alg_logger.info(' -----> (4) Resample datasets ... ')
                # resample datasets
                ...
                alg_logger.info(' -----> (4) Resample datasets ... DONE')

                # info start compose datasets
                alg_logger.info(' -----> (5) Compose datasets ... ')
                # compose datasets
                obj_data_anc_compose_step, obj_attrs_anc_compose_step = compose_data(
                    obj_data_anc_resample_step, obj_attrs_anc_step, obj_time_anc, settings_compose_data,
                    geo_terrain_ref=grid_geo_values_dst, geo_x_ref=grid_geo_x_dst, geo_y_ref=grid_geo_y_dst)
                # info end compose datasets
                alg_logger.info(' -----> (5) Compose datasets ... DONE')

                # info start mask datasets
                alg_logger.info(' -----> (6) Mask datasets ... ')
                # mask datasets
                obj_data_anc_mask_step = mask_data(obj_data_anc_compose_step, **settings_mask_data)

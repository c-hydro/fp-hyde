...
# ----------------------------------------------------------------------------------------
# Script information
script_name='HYDE DOWNLOADER - HSAF PRODUCT SOIL MOISTURE RZSM - H14'
script_version="2.0.1"
script_date='2019/07/04'
...
# Define folder(s)
folder_in_raw="/home/hsaf/hsaf_datasets/dynamic/source/h14/%YYYY/%MM/%DD/"
folder_tmp_raw="/home/hsaf/hsaf_datasets/dynamic/source/h14/%YYYY/%MM/%DD/"
folder_out_raw="/home/hsaf/hsaf_datasets/dynamic/source/rzsm/%YYYY/%MM/%DD/"
folder_grid="/home/hsaf/hsaf_datasets/static/soil_moisture/gridded/mod_rzsm/nrt/grid_domain/h14/"
		...
		if [[ $path_in_bz2 == *.bz2 ]]; then
			...
			if ! [ -e ${path_out_nc_step} ]; then
				...
				if ! [ -e ${path_out_nc_step} ]; then
				    ...
				    if ${exec_cdo} sellonlatbox,${domain_bb} ${path_in_nc_step} ${path_out_nc_step} > /dev/null 2>&1; then
				        echo " DONE!"
				    else
				        echo " FAILED! Error in command execution!"
				    fi

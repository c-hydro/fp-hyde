...
from lib_utils_io import read_file_tif, write_file_tif, write_obj
...
class DriverData:
        ...
        if self.file_src:
            ...
            for file_month, file_list in self.file_src.items():
                ...
                with warnings.catch_warnings():
                    ...
                    file_data_avg = np.nanmean(file_data_tmp, axis=2)
    ...
    def composer_data(self, ws_obj=None):
        ...
        file_stats_vars = [self.mean_tag, self.stdev_tag]
        ...
        if ws_obj is not None:
            ...
            for month_n_id in self.month_n_ref:
                ...
                values_avg = stats_dict[self.mean_tag]
                values_std = stats_dict[self.stdev_tag]

                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    values_index = (values_ref - geo_fc) / (geo_fc - geo_wp)
                    values_norm = (values_index - values_avg) / values_std

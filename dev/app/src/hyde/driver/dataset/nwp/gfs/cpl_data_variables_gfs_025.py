...
Name:          cpl_data_variables_gfs_025
...
import src.hyde.dataset.nwp.gfs.lib_gfs_025_time as var_fx_time
import src.hyde.dataset.nwp.gfs.lib_gfs_025_variables as var_fx_archive

from src.hyde.algorithm.io.nwp.gfs.lib_gfs_io_generic import create_darray_2d, create_darray_3d
from src.hyde.algorithm.settings.nwp.gfs.lib_gfs_args import logger_name, time_units, time_format, time_calendar
...
class DataVariables:
    ...
    def __init__(self, info_key, var_dset, var_geo_x, var_geo_y,
        ...
        if 'file_tag_time' in kwargs:
            file_tag_time = kwargs['file_tag_time']
        else:
            file_tag_time = 'time'
        if 'file_tag_geo_x' in kwargs:
            file_tag_geo_x = kwargs['file_tag_geo_x']
        else:
            file_tag_geo_x = 'longitude'
        if 'file_tag_geo_y' in kwargs:
            file_tag_geo_y = kwargs['file_tag_geo_y']
        else:
            file_tag_geo_y = 'latitude'
        ...
        self.var_dset_name = var_name_tmp
    ...
    def configure_fx(self):
        ...
        var_fx = list(set(self.var_fx))

        var_dset_data = self.var_dset_data
        var_dset_time = var_dset_data[self.tag_time]
        ...
        obj_fx_frame = None
        for name_fx, name_var in zip(var_fx, list(var_dset_name)):
            # Starting info
            log_stream.info(' ----> Apply computing function(s) to ' + name_var + ' variable ... ')
            ...
            if name_fx is not None:
                ...
                obj_fx_dictionary = dict(var_dset=var_dset_data, var_name=var_dset_name, var_units=var_dset_units,
                                         var_step_type=var_dset_step_type, geo_dset=geo_dset,
                                         var_time=[self.file_tag_time],
                                         var_geo_x=[self.file_tag_geo_x], var_geo_y=[self.file_tag_geo_y])
                ...
                log_stream.info(' ----> Apply computing function(s) to ' + name_var + ' variable ... DONE')

            else:
                # Ending info for undefined function
                log_stream.info(' ----> Apply computing function(s) to ' + name_var + ' variable ... SKIPPED.')

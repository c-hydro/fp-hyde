...
from src.hyde.algorithm.io.nwp.gfs.lib_gfs_io_generic import create_darray_3d, create_dset, write_dset
from src.hyde.algorithm.io.nwp.gfs.lib_gfs_io_nc import read_data_gfs_025 as read_data_gfs_025_nc
# from src.hyde.algorithm.io.nwp.gfs.lib_gfs_io_grib import read_data_gfs_025 as read_data_gfs_025_grib
...
class DataReader:
    ...
    def __init__(self,
                 ...
                 file_tag_geo_x='lon', file_tag_geo_y='lat', file_tag_time='time'):
            ...
            for file_id, (var_source, var_format, var_dims, var_name, file_name) in enumerate(
                ...
                if (file_name is not None) and (os.path.exists(file_name)):
                    ...
                    if var_format == 'netcdf':
                        ...
                        if var_source == 'gfs_025':
                            ...
                            if var_dims == 'var2d':
                                ...
                                [file_data_raw, _,
                                 file_geo_x, file_geo_y] = read_data_gfs_025_nc(
                                    file_name,
                                    tag_geo_x=self.file_tag_geo_x, tag_geo_y=self.file_tag_geo_y,
                                    tag_time=self.file_tag_time)
                    ...
                    if var_key in list(var_frame.keys()):
                        ...
                        var_time_null = var_frame_step[self.file_tag_time]
                        var_geo_x_null = var_frame_step[self.file_tag_geo_x].values
                        var_geo_y_null = var_frame_step[self.file_tag_geo_y].values

                        var_da_null = create_darray_3d(
                            var_frame_null, var_time_null, var_geo_x_null, var_geo_y_null,
                            dim_key_time=self.file_tag_time,
                            dim_key_x=self.file_tag_geo_x, dim_key_y=self.file_tag_geo_y,
                            dim_name_x=self.file_tag_geo_x, dim_name_y=self.file_tag_geo_y,
                            dim_name_time=self.file_tag_time,
                            dims_order=[self.file_tag_time, self.file_tag_geo_y, self.file_tag_geo_x])

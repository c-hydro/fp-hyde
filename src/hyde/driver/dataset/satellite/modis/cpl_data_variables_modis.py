...
import src.hyde.dataset.satellite.modis.lib_modis_variables as fx_collections
...
class DriverVariable:
    ...
    def __init__(self, var_obj, ref_obj, var_attributes=None,
                 ...
                 tag_var_geo_x='west_east', tag_var_geo_y='south_north', tag_var_geo_z='altitude',
                 tag_ref_geo_x='west_east', tag_ref_geo_y='south_north', tag_ref_geo_data='terrain',
                 tag_var_data='data'
                 ):
        ...
        self.tag_var_data = tag_var_data

        self.tag_ref_geo_x = tag_ref_geo_x
        self.tag_ref_geo_y = tag_ref_geo_y
        self.tag_ref_geo_data = tag_ref_geo_data
        ...
        self.fx_nodata, self.fx_flag_masks, \
            self.fx_flag_values, self.fx_flag_meanings, self.fx_pixel_size_aggregation = self.get_fx_attributes()
    ...
    def filter_var_valid_range(self, var_dset, var_nodata=-2):

        var_data = var_dset[self.tag_var_data].values

        if self.var_valid_range is not None:
            if self.var_valid_range[0] is not None:
                var_data[var_data < self.var_valid_range[0]] = var_nodata
            if self.var_valid_range[1] is not None:
                var_data[var_data > self.var_valid_range[1]] = var_nodata
            var_dset[self.tag_var_data].values = var_data
        else:
    ...
    def get_fx_attributes(self):
        ...
        fx_nodata = -1
        if 'nodata' in list(self.fx_parameters.keys()):
            fx_nodata = self.fx_parameters['nodata']
        fx_flag_masks = None
        if 'flag_masks' in list(self.fx_parameters.keys()):
            fx_flag_masks = self.fx_parameters['flag_masks']
        fx_flag_values = None
        if 'flag_values' in list(self.fx_parameters.keys()):
            fx_flag_values = self.fx_parameters['flag_values']
        fx_flag_meanings = None
        if 'flag_meanings' in list(self.fx_parameters.keys()):
            fx_flag_meanings = self.fx_parameters['flag_meanings']
        fx_pixel_size_aggregation = None
        if 'pixel_size_aggregation' in list(self.fx_parameters.keys()):
            fx_pixel_size_aggregation = self.fx_parameters['pixel_size_aggregation']

        return fx_nodata, fx_flag_masks, fx_flag_values, fx_flag_meanings, fx_pixel_size_aggregation
    ...
    def compute_data(self, ref_geo_index=None):
        ...
        if self.fx_obj:
            ...
            # Create fx data
            fx_data = {'var_data': var_obj[self.tag_var_data].values,
                       ...
                       'fx_nodata': self.fx_nodata, 'fx_flag_masks': self.fx_flag_masks,
                       'fx_flag_values': self.fx_flag_values, 'fx_flag_meanings': self.fx_flag_meanings,
                       'fx_pixel_size_aggregation': self.fx_pixel_size_aggregation,
                       'ref_geo_x': ref_obj[self.tag_ref_geo_x].values,
                       'ref_geo_y': ref_obj[self.tag_ref_geo_y].values,
                       'ref_geo_z': ref_obj[self.tag_ref_geo_data].values,
                       'ref_cell_size': np.mean([self.ref_cellsize_x, self.ref_cellsize_y]),
                       'ref_no_data': self.ref_nodata, 'ref_epsg': '4326',
                       'ref_geo_index': ref_geo_index
                       }

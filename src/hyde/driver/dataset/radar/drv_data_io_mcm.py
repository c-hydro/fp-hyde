...
from numpy import reshape, full, empty, nan, zeros, isnan, asarray, where

from src.common.analysis.lib_analysis_interpolation_grid import interpGridData
...
from src.common.utils.lib_utils_apps_file import selectFileDriver, zipFileData
...
from src.hyde.dataset.radar.lib_mcm_io_utils import readData
                    ...
                    for iVarID, (sVarType_IN, sVarName_IN, sVarFile_IN, sVarMethod_IN,
                        ...
                        if sVarName_IN in oVarWS[sVarKey]:
                            ...
                            if ('values' in oVarData) and ('attributes' in oVarData) and (
                                ...
                                a2dVarValue_IN = deepcopy(oVarData['values']).astype(float)
                                a1dVarGeoX_IN = deepcopy(oVarData['longitude']).astype(float)
                                a1dVarGeoY_IN = deepcopy(oVarData['latitude']).astype(float)
                                ...
                                if 'units' in oVarAttrs_IN_SEL:
                                    assert oVarAttrs_IN_SEL['units'] == 'mm/h'
                                ...
                                # Check data attributes to control units conversion
                                if 'units' in oVarAttrs_OUT_SEL:
                                    assert oVarAttrs_OUT_SEL['units'] == 'mm/h'
                                    assert oVarAttrs_OUT_SEL['ScaleFactor'] == 1
                                else:
                                ...
                                for iTimeStep, sTimeStep in enumerate(oPBar(oVarTime)):
                                    ...
                                    a1dVarValue_IN = a2dVarValue_IN[:, iTimeStep]
                                    # -------------------------------------------------------------------------------------

                                    # -------------------------------------------------------------------------------------
                                    # Set nan for missing and outbounds values
                                    a1dVarValue_IN[a1dVarValue_IN == dVarMissValue] = nan
                                    if oVarValidRange[0] is not None:
                                        a1dVarValue_IN[a1dVarValue_IN < oVarValidRange[0]] = nan
                                    if oVarValidRange[1] is not None:
                                        a1dVarValue_IN[a1dVarValue_IN > oVarValidRange[1]] = nan
                                    # -------------------------------------------------------------------------------------

                                    # -------------------------------------------------------------------------------------
                                    # Interpolate variable over grid reference using nearest method (gdal library)
                                    a2dVarValue_INTERP = interpGridData(
                                        a1dVarValue_IN, a1dVarGeoX_IN, a1dVarGeoY_IN,
                ...
                for iVarTime, sVarTime in enumerate(self.oVarTime):
                    ...
                    for iVarID, (sVarType, sVarName, sVarFile, sVarMethod) in enumerate(zip(
                        ...
                        if exists(sVarFileName):
                            ...
                            # -------------------------------------------------------------------------------------
                            # Get data
                            oFileObj, bFileStatus = readData(sVarFileName)
                            oVarAttribute = selectVarAttributes(self.oVarData[sVarKey], iVarID)
                            # -------------------------------------------------------------------------------------

                            # -------------------------------------------------------------------------------------
                            # Check file opening
                            if bFileStatus is True:
                                ...
                                assert dVarMissingValue == -9999.0

                                a1dVarData = oFileObj['data'].ravel()
                                a1dVarGeoX = oFileObj['longitude'][:].ravel()
                                a1dVarGeoY = oFileObj['latitude'][:].ravel()

                                a1iVarData_Idx = where(a1dVarData == dVarMissingValue)[0]

                                a1dVarData[a1iVarData_Idx] = nan
                                # -------------------------------------------------------------------------------------
                                ...
                                if oVarGet[sVarName]['longitude'] is None:
                                    a1dVarGeoX_DEF = empty([a1dVarData.shape[0]])
                                    a1dVarGeoX_DEF[:] = nan
                                    a1dVarGeoX_DEF[:] = a1dVarGeoX
                                    oVarGet[sVarName]['longitude'] = a1dVarGeoX_DEF

                                # Save variable latitude
                                if oVarGet[sVarName]['latitude'] is None:
                                    a1dVarGeoY_DEF = empty([a1dVarData.shape[0]])
                                    a1dVarGeoY_DEF[:] = nan
                                    a1dVarGeoY_DEF[:] = a1dVarGeoY
                                    oVarGet[sVarName]['latitude'] = a1dVarGeoY_DEF
                                # -------------------------------------------------------------------------------------

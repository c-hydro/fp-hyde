...
Name:          lib_ws_analysis_regression_stepwisefit
...
def stepcalc(allx, y, inmodel):
    ...
    P = inmodel.size    # Number of independent variables in each test (cols in allx).
...
def stepwisefit(allx, y, inmodel=None, penter=0.05, premove=0.10,
                display=False, maxiter=np.Inf, keep=None, scale=False):
    ...
    while True:
        ...
        if not scale:
            ...
            stats.b = b
            stats.se = se
            stats.xr = stats.xr * np.tile(sx[:, ~inmodel], (stats.xr.shape[0], 1))
        ...
        if nextstep == -1:
            ...
            print("Step %d, %s column %d, p=%f" % (jstep, addremoved, nextstep, pinout))
